...
async function migrateAdminEmail(fromEmail, toEmail) {
  const existing = await findUserByEmail(fromEmail)
  ...
  console.log(`Migrated admin login from ${fromEmail} to ${toEmail}`)
  return existing.id
}
...
if (previousEmail && previousEmail !== email) {
  try {
    userId = await migrateAdminEmail(previousEmail, email)
  } catch (err) {
    console.error('Admin email migration failed:', err.message)
    process.exit(1)
  }
}
